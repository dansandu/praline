    ...
    def __init__(self, file_system: FileSystem, artifact_manifest: ArtifactManifest):
        ...
        if artifact_manifest.platform != Platform.windows:
            raise CompilerInstantionError(
                f"the msvc compiler cannot be used on the '{artifact_manifest.platform}' platform")
        
        if not file_system.exists(self.environment_file):
            raise CompilerInstantionError(f"the msvc compiler could not find environment configuration batch file")

        if file_system.which('cl') == None:
            raise CompilerInstantionError(f"the msvc compiler could not find the cl executable in the PATH")

        if artifact_manifest.exported_symbols == ExportedSymbols.all:
            raise CompilerInstantionError(f"the msvc compiler does not support currently exporting all symbols")
...
class MsvcCompilerSupplier(CompilerSupplier):
