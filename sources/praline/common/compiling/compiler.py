...
def compile_using_cache(file_system: FileSystem,
    ...
    with progress_bar_supplier.create(resolution) as progress_bar:
        def hasher(source: str):
            ...
            return hash_binary(compiler.preprocess(project_structure.sources_root, 
        ...
        for item in delta(sources, hasher, cache, new_cache):
            ...
            if item.delta_type in [DeltaType.Added, DeltaType.Modified]:
                compiler.compile(project_structure.sources_root, 
                                 project_structure.external_headers_root, 
                                 headers, 
                                 source, 
                                 object_)
...
def link_executable_using_cache(file_system: FileSystem,
    ...
    yield_descriptor = compiler.get_yield_descriptor()
    executable       = yield_descriptor.get_executable(project_structure.executables_root, artifact_identifier)
    symbols_table    = yield_descriptor.get_symbols_table(project_structure.symbols_tables_root, artifact_identifier)
    compiler.link_executable(project_structure.external_libraries_root,
                             ...
                             executable,
                             symbols_table)
    if file_system.exists(symbols_table):
...
def link_library_using_cache(file_system: FileSystem,
    ...
    yield_descriptor  = compiler.get_yield_descriptor()
    library           = yield_descriptor.get_library(project_structure.libraries_root, artifact_identifier)
    library_interface = yield_descriptor.get_library_interface(project_structure.libraries_interfaces_root, artifact_identifier)
    symbols_table     = yield_descriptor.get_symbols_table(project_structure.symbols_tables_root, artifact_identifier)
    compiler.link_library(project_structure.external_libraries_root,
                          project_structure.external_libraries_interfaces_root,
                          objects,
                          external_libraries,
                          external_libraries_interfaces,
...
def get_compiler_suppliers() -> List[CompilerSupplier]:
    compiler_suppliers = [klass() for klass in subclasses_of(CompilerSupplier)]
...
def get_compiler_supplier(name: Compiler) -> CompilerSupplier:
...
def intantiate_compiler(file_system: FileSystem,
                        ...
                        fallback_compilers: List[Compiler]) -> Tuple[ArtifactManifest, ICompiler]:
    ...
    return (final_manifest, compiler)
