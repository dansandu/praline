...
def validate_platforms(pralinefile: Dict[str, Any]):
    ...
    if not platforms:
        raise PralinefileValidationException(f"Pralinefile platforms field cannot be empty")
    converted_platforms = []
    for platform in platforms:
        if platform not in allowed_platforms:
            raise PralinefileValidationException(f"Pralinefile platform '{platform}' is not recognized -- allowed "
