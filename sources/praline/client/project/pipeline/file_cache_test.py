...
class CacheTest(TestCase):
    ...
    def test_non_active_cache(self):
        file_path   = join('my', 'cache')
        file_system = FileSystemMock()

        with FileCache(file_system, file_path, active=False) as cache:
            cache['blue'] = 'sky'
        
        self.assertEqual(len(file_system.files), 0)
        
        self.assertEqual(len(file_system.directories), 0)
