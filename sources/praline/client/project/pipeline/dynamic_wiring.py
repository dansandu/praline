...
def run_unit_tests_wiring(stages):
    if any(f for f in files_in_directory(source_root) if f.endswith('.test.cpp')):
        stages['package_executable'].consumes.extend(stages['run_test_executable'].produces)
        stages['package_header_only'].consumes.extend(stages['run_test_executable'].produces)
        stages['package_library'].consumes.extend(stages['run_test_executable'].produces)
        stages['run_main_executable'].consumes.extend(stages['run_test_executable'].produces)
