...
def predicate(arguments: StagePredicateArguments):
    ...
    if any_farseer_sources:
        ...
        return StagePredicateResult.failure("there are no test farseer sources")


@stage(requirements=[['project_directories']], output=['test_farseer_sources'], predicate=predicate)
def load_test_farseer_sources(arguments: StageArguments):
    file_system       = arguments.file_system
