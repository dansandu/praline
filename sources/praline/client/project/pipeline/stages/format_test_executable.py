from praline.client.project.pipeline.stages import StageArguments, stage
from praline.common.hashing import DeltaItem, DeltaType, delta, hash_file, progression_resolution


@stage(requirements=[['clang_format_executable', 'test_executable_source']],
       output=['formatted_test_executable_source'],
       has_progress_bar=True)
def format_test_executable(arguments: StageArguments):
    resources   = arguments.resources
    file_system = arguments.file_system
    cache       = arguments.cache
    
    progress_bar_supplier = arguments.progress_bar_supplier

    test_executable_source = resources['test_executable_source']
    clang_format           = resources['clang_format_executable']

    hasher    = lambda f: hash_file(file_system, f)
    new_cache = {}

    resolution = progression_resolution([test_executable_source], cache)
    with progress_bar_supplier.create(resolution) as progress_bar:
        def consumer(item: DeltaItem):
            test_source = item.key
            if item.delta_type in [DeltaType.Added, DeltaType.Modified]:
                progress_bar.update_description(test_source)
                file_system.execute_and_fail_on_bad_return([clang_format, '-i', '-style=file', test_source])
            progress_bar.advance()

        delta([test_executable_source], hasher, cache, new_cache, consumer)

    resources['formatted_test_executable_source'] = resources['test_executable_source']

    cache.clear()
    cache.update(new_cache)
